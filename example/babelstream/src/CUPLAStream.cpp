...
template<class T>
CUPLAStream<T>::CUPLAStream(int const ARRAY_SIZE, int const device_index)
{
...
struct init_kernel
{
    ...
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T* a, T* b, T* c, const T initA, const T initB, const T initC)
        const
    {
...
{
    template<typename T_Acc>
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T const* a, T* c) const
    {
        int const i = blockDim.x * blockIdx.x + threadIdx.x;
        c[i] = a[i];
    }
...
{
    template<typename T_Acc>
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T* b, T const* c) const
    {
...
{
    template<typename T_Acc>
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T const* a, T const* b, T* c) const
    {
        int const i = blockDim.x * blockIdx.x + threadIdx.x;
        c[i] = a[i] + b[i];
    }
...
{
    template<typename T_Acc>
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T* a, T const* b, T const* c) const
    {
        const T scalar = startScalar;
        int const i = blockDim.x * blockIdx.x + threadIdx.x;
        a[i] = b[i] + scalar * c[i];
...
{
    template<typename T_Acc>
    ALPAKA_FN_ACC void operator()(T_Acc const& acc, T const* a, T const* b, T* sum, int const array_size) const
    {
...
std::string getDeviceName(int const device)
{
    // cudaDeviceProp props;
...
std::string getDeviceDriver(int const device)
{
