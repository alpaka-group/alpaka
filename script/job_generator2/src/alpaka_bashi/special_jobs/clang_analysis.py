...
def get_clang_debug_analysis_job(
    ...
    job_body = construct_job_yaml(
        combination=Combination(
            {
                HOST_COMPILER: ParameterValue(CLANG, packaging.version.parse(clang_version)),
                DEVICE_COMPILER: ParameterValue(CLANG, packaging.version.parse(clang_version)),
                ...
                JOB_EXECUTION_TYPE: ParameterValue(JOB_EXECUTION_TYPE, JOB_EXECUTION_RUNTIME_VER),
                ...
                ALPAKA_ACC_GPU_CUDA_ENABLE: ParameterValue(ALPAKA_ACC_GPU_CUDA_ENABLE, OFF_VER),
        ...
        combination=Combination(
            {
                ...
                CMAKE: ParameterValue(CMAKE, packaging.version.parse(cmake_version)),
                UBUNTU: ParameterValue(UBUNTU, packaging.version.parse("24.04")),
                CXX_STANDARD: ParameterValue(CXX_STANDARD, packaging.version.parse(cpp_standard)),
                BUILD_TYPE: ParameterValue(BUILD_TYPE, CMAKE_RELEASE_WITH_DEBUG_INFO_VER),
                MDSPAN: ParameterValue(MDSPAN, OFF_VER),
                JOB_EXECUTION_TYPE: ParameterValue(JOB_EXECUTION_TYPE, JOB_EXECUTION_RUNTIME_VER),
                CI_PIPELINE_NAME: ParameterValue(CI_PIPELINE_NAME, CI_PIPELINE_SPECIAL_VER),
                ALPAKA_ACC_CPU_B_SEQ_T_SEQ_ENABLE: ParameterValue(
