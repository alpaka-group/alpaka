...
class TestAlpakaGetValidBackendCombinations(unittest.TestCase):
    def test_empty_row_empty_backend_list(self):
        self.assertTrue(alpaka_bashi.filter.all_backends_fine(OrderedDict(), [], []))

    def test_empty_row_single_backend(self):
        self.assertTrue(
            alpaka_bashi.filter.all_backends_fine(
                row=OrderedDict(),
    ...
    def test_empty_row_two_backend(self):
        for enabled_backends, all_backends in [
            (
            ...
            self.assertTrue(
                alpaka_bashi.filter.all_backends_fine(OrderedDict(), enabled_backends, all_backends)
            )
    ...
    def test_multiple_row_multiple_backend_valid(self):
        for row, backends, all_backends in [
            (
            ...
            self.assertTrue(
                alpaka_bashi.filter.all_backends_fine(row, backends, all_backends),
                f"\nrow:\n  {row}\nbackends:\n  {backends}\nall backends:  \n{all_backends}",
            )
