...
TEMPLATE_LIST_TEST_CASE("queueWaitShouldWork", "[queue]", TestQueues)
{
    ...
    alpaka::enqueue(f.m_queue, [&callback_finished]() noexcept {
    ...
    TestQueues)
{
    ...
    std::atomic<bool> callback_finished{false};
    alpaka::enqueue(f.m_queue, [&f, &callback_finished]() noexcept {
...
TEMPLATE_LIST_TEST_CASE("queueShouldNotExecuteTasksInParallel", "[queue]", TestQueues)
{
    ...
    std::thread thread1([&f, &task_is_executing, &first_task_finished]() {
        alpaka::enqueue(f.m_queue, [&task_is_executing, &first_task_finished]() noexcept {
    ...
    std::thread thread2([&f, &task_is_executing, &second_task_finished]() {
        alpaka::enqueue(f.m_queue, [&task_is_executing, &second_task_finished]() noexcept {
            CHECK(!task_is_executing.exchange(true));
            std::this_thread::sleep_for(std::chrono::milliseconds(100u));
            CHECK(task_is_executing.exchange(false));
