...
extern ALPAKA_STATIC_ACC_MEM_CONSTANT Elem g_constant_memory2_d_initialized[3][2];
extern ALPAKA_STATIC_ACC_MEM_CONSTANT Elem g_constant_memory2_d_uninitialized[3][2];

ALPAKA_STATIC_ACC_MEM_CONSTANT Elem g_constant_memory2_d_initialized[3][2] = {{0u, 1u}, {2u, 3u}, {4u, 5u}};
...
// FIXME: static device memory in HIP is still not working
    ...
    {
        auto const view_constant_mem_initialized
            = alpaka::createStaticDevMemView(&g_constant_memory2_d_initialized[0u][0u], dev_acc, extent);

        REQUIRE(fixture(kernel, alpaka::getPtrNative(view_constant_mem_initialized)));
    }
    // uninitialized static constant device memory
    {
        ...
        alpaka::ViewPlainPtr<decltype(dev_host), const Elem, Dim, Idx> buf_host(data.data(), dev_host, extent);
...
// Here they are used to silence clang`s -Wmissing-variable-declarations warning
// that forces every non-static variable to be declared with extern before the are defined.
extern ALPAKA_STATIC_ACC_MEM_GLOBAL Elem g_global_memory2_d_initialized[3][2];
extern ALPAKA_STATIC_ACC_MEM_GLOBAL Elem g_global_memory2_d_uninitialized[3][2];

ALPAKA_STATIC_ACC_MEM_GLOBAL Elem g_global_memory2_d_initialized[3][2] = {{0u, 1u}, {2u, 3u}, {4u, 5u}};
...
// FIXME: static device memory in HIP is still not working
    ...
    {
        auto const view_global_mem_initialized
            = alpaka::createStaticDevMemView(&g_global_memory2_d_initialized[0u][0u], dev_acc, extent);

        REQUIRE(fixture(kernel, alpaka::getPtrNative(view_global_mem_initialized)));
    }
