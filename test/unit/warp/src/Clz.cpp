...
struct ClzSingleThreadWarpTestKernel
{
    ...
    ALPAKA_FN_ACC auto operator()(TAcc const& acc, bool* success) const -> void
    {
        ...
        ALPAKA_CHECK(*success, alpaka::warp::clz(acc, 1) == 0);
    }
};

template<std::uint32_t TWarpSize>
struct ClzMultipleThreadWarpTestKernel
{
    ALPAKA_NO_HOST_ACC_WARNING
    template<typename TAcc>
    ALPAKA_FN_ACC auto operator()(TAcc const& acc, bool* success) const -> void
...
struct alpaka::trait::WarpSize<ClzMultipleThreadWarpTestKernel<TWarpSize>, TAcc>
    : std::integral_constant<std::uint32_t, TWarpSize>
{
};

TEMPLATE_LIST_TEST_CASE("clz", "[warp]", alpaka::test::TestAccs)
{
    using Acc = TestType;
    ...
    for(auto const warpExtent : warpExtents)
    {
        ...
        if(scalar)
        {
            ...
            REQUIRE(fixture(ClzSingleThreadWarpTestKernel{}));
        }
        else
        {
            ...
            if(warpExtent == 4)
            {
                REQUIRE(fixture(ClzMultipleThreadWarpTestKernel<4>{}));
            }
            else if(warpExtent == 8)
            {
                REQUIRE(fixture(ClzMultipleThreadWarpTestKernel<8>{}));
            }
            else if(warpExtent == 16)
            {
                REQUIRE(fixture(ClzMultipleThreadWarpTestKernel<16>{}));
            }
            else if(warpExtent == 32)
            {
                REQUIRE(fixture(ClzMultipleThreadWarpTestKernel<32>{}));
            }
            else if(warpExtent == 64)
            {
                REQUIRE(fixture(ClzMultipleThreadWarpTestKernel<64>{}));
            }
        }
    }
#endif
