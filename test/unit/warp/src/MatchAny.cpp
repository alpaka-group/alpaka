...
struct MatchAnySingleThreadWarpTestKernel
{
    ...
    ALPAKA_FN_ACC auto operator()(TAcc const& acc, bool* success) const -> void
    {
        ...
        ALPAKA_CHECK(*success, alpaka::warp::match_any(acc, 1, match_val) == 1);
    }
};

template<std::uint32_t TWarpSize>
struct MatchAnyMultipleThreadWarpTestKernel
{
    ALPAKA_NO_HOST_ACC_WARNING
    template<typename TAcc>
    ALPAKA_FN_ACC auto operator()(TAcc const& acc, bool* success) const -> void
    {
        ...
        ALPAKA_CHECK(*success, alpaka::warp::match_any(acc, mask, warpExtent) == mask);

        MaskType const match_mask = alpaka::warp::match_any(acc, mask, match_val);
...
struct alpaka::trait::WarpSize<MatchAnyMultipleThreadWarpTestKernel<TWarpSize>, TAcc>
    : std::integral_constant<std::uint32_t, TWarpSize>
{
};

TEMPLATE_LIST_TEST_CASE("match_any", "[warp]", alpaka::test::TestAccs)
{
    using Acc = TestType;
    ...
    for(auto const warpExtent : warpExtents)
    {
        ...
        if(scalar)
        {
            ...
            REQUIRE(fixture(MatchAnySingleThreadWarpTestKernel{}));
        }
        else
        {
            ...
            if(warpExtent == 4)
            {
                REQUIRE(fixture(MatchAnyMultipleThreadWarpTestKernel<4>{}));
            }
            else if(warpExtent == 8)
            {
                REQUIRE(fixture(MatchAnyMultipleThreadWarpTestKernel<8>{}));
            }
            else if(warpExtent == 16)
            {
                REQUIRE(fixture(MatchAnyMultipleThreadWarpTestKernel<16>{}));
            }
            else if(warpExtent == 32)
            {
                REQUIRE(fixture(MatchAnyMultipleThreadWarpTestKernel<32>{}));
            }
            else if(warpExtent == 64)
            {
                REQUIRE(fixture(MatchAnyMultipleThreadWarpTestKernel<64>{}));
            }
        }
    }
#endif
