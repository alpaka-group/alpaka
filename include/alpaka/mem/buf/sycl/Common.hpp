...
namespace alpaka::experimental::detail
{
    ...
    inline auto make_sycl_range(TExtent const& ext, std::size_t multiplier = 1)
    {
        ...
        const auto width = extent::getWidth(ext) * multiplier;

        if constexpr(dim == 1)
            return sycl::range<1>{width};
        else if constexpr(dim == 2)
            return sycl::range<2>{width, extent::getHeight(ext)};
        else
            return sycl::range<3>{width, extent::getHeight(ext), extent::getDepth(ext)};
    }
