                ...
                if(!isPinned(buf))
                {
                    if(buf.m_spBufCpuImpl->m_extentElements.prod() != 0)
                    {
                        ...
                        ALPAKA_CUDA_RT_CHECK_IGNORE(
                            cudaHostRegister(
                                const_cast<void*>(reinterpret_cast<void const*>(buf.data())),
...
#if defined(ALPAKA_ACC_GPU_CUDA_ENABLED)
                    ALPAKA_CUDA_RT_CHECK_IGNORE(
                        cudaHostUnregister(const_cast<void*>(reinterpret_cast<void const*>(bufImpl.m_pMem))),
                        cudaErrorHostMemoryNotRegistered);
#endif
#if defined(ALPAKA_ACC_GPU_HIP_ENABLED)
                    ALPAKA_HIP_RT_CHECK_IGNORE(
                        hipHostUnregister(const_cast<void*>(reinterpret_cast<void const*>(bufImpl.m_pMem))),
