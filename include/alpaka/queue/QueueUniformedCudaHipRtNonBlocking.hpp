    ...
    namespace queue
    {
        namespace uniformedCudaHip
        {
            namespace detail
            {
                ...
                class QueueUniformedCudaHipRtNonBlockingImpl final
                {
                public:
                    //-----------------------------------------------------------------------------
                    ALPAKA_FN_HOST QueueUniformedCudaHipRtNonBlockingImpl(
                    ...
                    //-----------------------------------------------------------------------------
                    QueueUniformedCudaHipRtNonBlockingImpl(QueueUniformedCudaHipRtNonBlockingImpl const &) = delete;
                    //-----------------------------------------------------------------------------
                    QueueUniformedCudaHipRtNonBlockingImpl(QueueUniformedCudaHipRtNonBlockingImpl &&) = default;
                    //-----------------------------------------------------------------------------
                    auto operator=(QueueUniformedCudaHipRtNonBlockingImpl const &) -> QueueUniformedCudaHipRtNonBlockingImpl & = delete;
                    //-----------------------------------------------------------------------------
                    auto operator=(QueueUniformedCudaHipRtNonBlockingImpl &&) -> QueueUniformedCudaHipRtNonBlockingImpl & = delete;
                    //-----------------------------------------------------------------------------
                    ALPAKA_FN_HOST ~QueueUniformedCudaHipRtNonBlockingImpl()
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
        ...
        class QueueUniformedCudaHipRtNonBlocking final : public concepts::Implements<wait::ConceptCurrentThreadWaitFor, QueueUniformedCudaHipRtNonBlocking>
        {
        public:
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST QueueUniformedCudaHipRtNonBlocking(
                dev::DevUniformedCudaHipRt const & dev) :
                m_spQueueImpl(std::make_shared<uniformedCudaHip::detail::QueueUniformedCudaHipRtNonBlockingImpl>(dev))
            {}
            //-----------------------------------------------------------------------------
            QueueUniformedCudaHipRtNonBlocking(QueueUniformedCudaHipRtNonBlocking const &) = default;
            //-----------------------------------------------------------------------------
            QueueUniformedCudaHipRtNonBlocking(QueueUniformedCudaHipRtNonBlocking &&) = default;
            //-----------------------------------------------------------------------------
            auto operator=(QueueUniformedCudaHipRtNonBlocking const &) -> QueueUniformedCudaHipRtNonBlocking & = default;
            //-----------------------------------------------------------------------------
            auto operator=(QueueUniformedCudaHipRtNonBlocking &&) -> QueueUniformedCudaHipRtNonBlocking & = default;
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST auto operator==(QueueUniformedCudaHipRtNonBlocking const & rhs) const
            -> bool
            {
            ...
            //-----------------------------------------------------------------------------
#if defined(ALPAKA_ACC_GPU_CUDA_ENABLED)
            ~QueueUniformedCudaHipRtNonBlocking() = default;
#else
            ~QueueUniformedCudaHipRtNonBlocking() {
#if BOOST_COMP_HCC  // NOTE: workaround for unwanted nonblocking hip streams for HCC (NVCC streams are blocking)
...
#endif
        public:
            std::shared_ptr<uniformedCudaHip::detail::QueueUniformedCudaHipRtNonBlockingImpl> m_spQueueImpl;
        };
    }
        ...
        namespace traits
        {
                ...
                queue::QueueUniformedCudaHipRtNonBlocking>
            {
                ...
                ALPAKA_FN_HOST static auto getDev(
                    queue::QueueUniformedCudaHipRtNonBlocking const & queue)
