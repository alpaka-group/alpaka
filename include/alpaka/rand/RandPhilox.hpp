    ...
    namespace rand
    {
        ...
        public:
            ...
            ALPAKA_FN_HOST_ACC TResult operator()(TEngine& engine)
            {
                ...
                auto result = engine();
                TResult ret{
                    static_cast<T>(result[0]) / engine.max() * _range + _min,
                    static_cast<T>(result[1]) / engine.max() * _range + _min,
                    static_cast<T>(result[2]) / engine.max() * _range + _min,
                    static_cast<T>(result[3]) / engine.max() * _range + _min};
                return ret;
            }
        ...
        /// TEMP: Distributions to be decided on later. The generator should be compatible with STL as of now.
        template<typename TResult>
        class UniformReal<TResult, std::enable_if_t<std::is_floating_point_v<TResult>>>
            : public concepts::Implements<ConceptRand, UniformReal<TResult>>
        {
            using T = TResult;
        ...
        public:
            ...
            {
                return static_cast<T>(engine()) / engine.max() * _range + _min;
            }
