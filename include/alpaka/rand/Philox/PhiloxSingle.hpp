...
namespace alpaka
{
    namespace rand
    {
        namespace engine
        {
            ...
            public:
                using Trait =
                    typename traits::PhiloxBaseTraits<TAcc, TParams, PhiloxSingle<TAcc, TParams>>; ///< Specialization
                                                                                                   ///< for different
                                                                                                   ///< backends
...
#ifdef PLAIN_GPU_VECTOR_TYPES
                    switch(state.position)
                    {
                    case 0:
                        return state.result.x;
                    case 1:
                        return state.result.y;
                    case 2:
                        return state.result.z;
                    case 3:
                        return state.result.w;
            ...
            public:
                ...
                ALPAKA_FN_HOST_ACC PhiloxSingle(uint64_t seed = 0, uint64_t subsequence = 0, uint64_t offset = 0)
                    : state{
                        {0, 0, 0, 0},
                        {static_cast<uint32_t>(lo32(seed)), static_cast<uint32_t>(hi32(seed))},
                        {0, 0, 0, 0},
                        0}
                {
