 ...
 */
 template <concepts::Acc TAcc>
 ALPAKA_FN_ACC ALPAKA_FN_INLINE auto get_ls1b_idx(TAcc const& acc, typename TAcc::mask_type const mask) -> typename TAcc::mask_type {
 ...
 */
 template <concepts::Acc TAcc>
 ALPAKA_FN_ACC ALPAKA_FN_INLINE bool is_ls1b_idx(typename TAcc::mask_type const mask, uint32_t const lane_idx) {
   using mask_t = typename TAcc::mask_type;
   ...
   const mask_t lane_mask = get_lane_mask<TAcc>(lane_idx);
   ...
   const mask_t inverted_mask = mask << (w_extent - lane_idx);
...
*/
 ...
 ALPAKA_FN_ACC ALPAKA_FN_INLINE std::uint32_t warp_exclusive_sum(TAcc const& acc,
   ...
   const mask_t full_mask = warp::ballot(acc, true);

   constexpr std::uint32_t w_extent = warp::getSizeCompileTime<TAcc>();

   const std::uint32_t local_offset = val;

   // Do inclusive sum first:
   //CMS_UNROLL_LOOP
   for (std::uint32_t step = 1; step < w_extent; step *= 2) {
     const auto res = warp::shfl_up(acc, local_offset, step, w_extent);
     if (lane_idx >= step)
       local_offset += res;
   }
   ...
   if constexpr (all) {
     ...
     const mask_t active_mask = 1 | get_lane_mask<TAcc>(high_lane_idx);
     const std::uint32_t tmp = warp::shfl(acc, active_mask, local_offset, high_lane_idx, w_extent);
...
*/
 ...
 template <concepts::Acc TAcc>
 ALPAKA_FN_ACC ALPAKA_FN_INLINE std::uint32_t get_logical_lane_idx(TAcc const& acc,
                                                                  typename TAcc::mask_type const mask,
                                                                  std::uint32_t const lane_idx) {
   if (lane_idx == 0)
     return lane_idx;  // nothing to do, phys idx coincide with the logical one.
   const auto lane_mask = mask & (get_lane_mask<TAcc>(lane_idx) - 1);
...
*/
 ...
 template <concepts::Acc TAcc>
 ALPAKA_FN_ACC ALPAKA_FN_INLINE std::uint32_t get_physical_lane_idx(TAcc const& acc,
                                                                   typename TAcc::mask_type const mask,
...
*/
 ...
 ALPAKA_FN_ACC ALPAKA_FN_INLINE auto warp_sparse_reduce(TAcc const& acc,
   ...
   const std::uint32_t pow2 = w_extent - warp::clz(acc, nActiveLanes - 1);
   const std::uint32_t pow2_boundary = 1 << pow2;

   const std::uint32_t logical_lane_idx = get_logical_lane_idx(acc, mask, lane_idx);
   ...
   for (std::uint32_t offset = pow2_boundary / 2; offset > 0; offset /= 2) {
     ...
     const reduce_t neigh_res = warp::shfl(acc, mask, res, src_lane_idx, w_extent);
   ...
   if constexpr (all) {
     ...
     res = warp::shfl(acc, mask, res, low_lane_idx, w_extent);
   }
...
*/
 ...
 ALPAKA_FN_ACC ALPAKA_FN_INLINE auto warp_sparse_exclusive_sum(TAcc const& acc,
   ...
   for (std::uint32_t step = 1; step < pow2_boundary; step *= 2) {
     ...
     const std::uint32_t tmp_val = warp::shfl(acc, mask, local_offset, src_lane_idx, w_extent);
   ...
   if constexpr (all) {
     const std::uint32_t high_lane_idx = get_physical_lane_idx(acc, mask, nActiveLanes - 1);
     // send last lane value (total tile offset) to lane idx = low_lane_idx:
     const std::uint32_t tmp = warp::shfl(acc, mask, local_offset, high_lane_idx, w_extent);
