...
namespace alpaka
{
    namespace event
    {
        namespace uniformedCudaHip
        {
            namespace detail
            {
                ...
                class EventUniformedCudaHipImpl final
                {
                public:
                    //-----------------------------------------------------------------------------
                    ALPAKA_FN_HOST EventUniformedCudaHipImpl(
                        dev::DevUniformedCudaHipRt const & dev,
                        bool bBusyWait) :
                            m_dev(dev),
                            m_UniformedCudaHipEvent()
                    {
                    ...
                    //-----------------------------------------------------------------------------
                    EventUniformedCudaHipImpl(EventUniformedCudaHipImpl const &) = delete;
                    //-----------------------------------------------------------------------------
                    EventUniformedCudaHipImpl(EventUniformedCudaHipImpl &&) = default;
                    //-----------------------------------------------------------------------------
                    auto operator=(EventUniformedCudaHipImpl const &) -> EventUniformedCudaHipImpl & = delete;
                    //-----------------------------------------------------------------------------
                    auto operator=(EventUniformedCudaHipImpl &&) -> EventUniformedCudaHipImpl & = delete;
                    //-----------------------------------------------------------------------------
                    ALPAKA_FN_HOST ~EventUniformedCudaHipImpl()
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
...
#if defined(ALPAKA_ACC_GPU_CUDA_ENABLED)
                    cudaEvent_t m_UniformedCudaHipEvent;
#else
                    hipEvent_t m_UniformedCudaHipEvent;
#endif
                };
            }
        }
        ...
        class EventUniformedCudaHipRt final : public concepts::Implements<wait::ConceptCurrentThreadWaitFor, EventUniformedCudaHipRt>
        {
        public:
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST EventUniformedCudaHipRt(
                dev::DevUniformedCudaHipRt const & dev,
                bool bBusyWait = true) :
                    m_spEventImpl(std::make_shared<uniformedCudaHip::detail::EventUniformedCudaHipImpl>(dev, bBusyWait))
            {
                ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
            }
            //-----------------------------------------------------------------------------
            EventUniformedCudaHipRt(EventUniformedCudaHipRt const &) = default;
            //-----------------------------------------------------------------------------
            EventUniformedCudaHipRt(EventUniformedCudaHipRt &&) = default;
            //-----------------------------------------------------------------------------
            auto operator=(EventUniformedCudaHipRt const &) -> EventUniformedCudaHipRt & = default;
            //-----------------------------------------------------------------------------
            auto operator=(EventUniformedCudaHipRt &&) -> EventUniformedCudaHipRt & = default;
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST auto operator==(EventUniformedCudaHipRt const & rhs) const
            -> bool
            {
#if defined(ALPAKA_ACC_GPU_CUDA_ENABLED)
                return (m_spEventImpl == rhs.m_spEventImpl);
#else
                return (m_spEventImpl->m_UniformedCudaHipEvent == rhs.m_spEventImpl->m_UniformedCudaHipEvent);
#endif
            }
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST auto operator!=(EventUniformedCudaHipRt const & rhs) const
            ...
            //-----------------------------------------------------------------------------
            ~EventUniformedCudaHipRt() = default;

        public:
            std::shared_ptr<uniformedCudaHip::detail::EventUniformedCudaHipImpl> m_spEventImpl;
        };
    }
    ...
    namespace dev
    {
        namespace traits
        {
            ...
            template<>
            struct GetDev<
                event::EventUniformedCudaHipRt>
            {
                //-----------------------------------------------------------------------------
                ALPAKA_FN_HOST static auto getDev(
                    event::EventUniformedCudaHipRt const & event)
                ...
                event::EventUniformedCudaHipRt>
            {
                ...
                -> bool
                {
                    ...
                    ALPAKA_CUDA_RT_CHECK_IGNORE(
                        ret = cudaEventQuery(
                            event.m_spEventImpl->m_UniformedCudaHipEvent),
                        cudaErrorNotReady);
        ...
        namespace traits
        {
                ...
                event::EventUniformedCudaHipRt>
            {
                ...
                ALPAKA_FN_HOST static auto enqueue(
                    ...
                    event::EventUniformedCudaHipRt & event)
                -> void
                {
                    ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
#if defined(ALPAKA_ACC_GPU_CUDA_ENABLED)
